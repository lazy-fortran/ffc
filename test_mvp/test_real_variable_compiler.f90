program test_real_variable_compiler
    ...
    character(len=*), parameter :: exe_path = '/tmp/ffc_real_variable_test'
    character(len=*), parameter :: out_path = '/tmp/ffc_real_variable_test.out'
    integer :: exit_stat
    integer :: io_stat
    integer :: unit
    ...
    print *, '=== real variable compiler test ==='
    ...
    if (io_stat /= 0 .or. trim(adjustl(output_line)) /= '3.500000') then
        print *, 'FAIL: expected output 3.500000, got ', trim(output_line)
        stop 1
    end if

    print *, 'PASS: real scalar variables and arithmetic'
end program test_real_variable_compiler
