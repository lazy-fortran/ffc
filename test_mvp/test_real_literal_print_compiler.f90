program test_real_literal_print_compiler
    ...
    character(len=*), parameter :: exe_path = '/tmp/ffc_real_literal_test'
    character(len=*), parameter :: out_path = '/tmp/ffc_real_literal_test.out'
    integer :: exit_stat
    integer :: io_stat
    integer :: unit
    ...
    print *, '=== real literal print compiler test ==='
    ...
    if (io_stat /= 0 .or. trim(adjustl(output_line)) /= '2.500000') then
        print *, 'FAIL: expected output 2.500000, got ', trim(output_line)
        stop 1
    end if

    print *, 'PASS: real literal print'
end program test_real_literal_print_compiler
