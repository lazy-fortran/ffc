program test_session_array_section_descriptor_compiler
    ...
    if (.not. expect_error_contains(forwarded_descriptor_source, &
        'assumed-shape descriptor forwarding is not yet supported', &
        trim(artifact_root)//'_forwarded_refused')) then
        call cleanup_artifact(trim(artifact_root)//'_forwarded_refused')
        stop 1
    end if
    call cleanup_artifact(trim(artifact_root)//'_forwarded_refused')
    ...
    call cleanup_artifact(trim(artifact_root)//'_side_effect_refused')
    if (.not. expect_error_contains(nested_descriptor_source, &
        'Rank mismatch in argument to inner', &
        trim(artifact_root)//'_nested_refused')) then
        call cleanup_artifact(trim(artifact_root)//'_nested_refused')
        stop 1
    end if
    call cleanup_artifact(trim(artifact_root)//'_nested_refused')
