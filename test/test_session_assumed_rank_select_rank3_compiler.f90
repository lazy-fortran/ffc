program test_session_assumed_rank_select_rank3_compiler
    use ffc_test_support, only: expect_output, expect_error_contains
    implicit none

    character(len=*), parameter :: source = &
        'program main'//new_line('a')// &
        '  real :: a(2,3,2)'//new_line('a')// &
        '  a = 0.0'//new_line('a')// &
        '  a(1,1,1) = 1.0'//new_line('a')// &
        '  a(2,1,1) = 2.0'//new_line('a')// &
        '  a(1,2,1) = 3.0'//new_line('a')// &
        '  a(2,2,1) = 4.0'//new_line('a')// &
        '  a(1,3,1) = 5.0'//new_line('a')// &
        '  a(2,3,1) = 6.0'//new_line('a')// &
        '  a(1,1,2) = 7.0'//new_line('a')// &
        '  a(2,1,2) = 8.0'//new_line('a')// &
        '  a(1,2,2) = 9.0'//new_line('a')// &
        '  a(2,2,2) = 10.0'//new_line('a')// &
        '  a(1,3,2) = 11.0'//new_line('a')// &
        '  a(2,3,2) = 12.0'//new_line('a')// &
        ...
        '    rank (3)'//new_line('a')// &
        '      x(2,1,1) = x(2,1,1) + 10.0'//new_line('a')// &
        '      x(1,2,1) = x(1,2,1) + 20.0'//new_line('a')// &
        '      x(2,3,2) = x(2,3,2) + 30.0'//new_line('a')// &
        '      print *, nint(x(1,1,1)), nint(x(2,1,1)), '// &
        'nint(x(1,2,1)), nint(x(2,3,2))'//new_line('a')// &
        '    end select'//new_line('a')// &
        '  end subroutine touch'//new_line('a')// &
        'end program main'

    character(len=*), parameter :: expected = &
        '           1          12          23          42'//new_line('a')

    print *, '=== direct session assumed-rank rank-3 SELECT RANK test ==='
    if (.not. expect_output(source, expected, &
            '/tmp/ffc_session_assumed_rank_select_rank3')) then
        print *, 'FAIL: ffc rank-3 behavioral case'
        stop 1
    end if
    if (.not. test_gfortran_oracle()) then
        print *, 'FAIL: independent gfortran differential oracle'
        stop 1
    end if
    if (.not. test_refusals()) then
        print *, 'FAIL: rank-3 refusal cases'
        stop 1
    end if
    print *, 'PASS: rank-3 REAL assumed-rank descriptor arm uses column-major access'

contains

    logical function test_gfortran_oracle()
        character(len=*), parameter :: src_path = &
            '/tmp/ffc_assumed_rank_select_rank3_oracle.f90'
        character(len=*), parameter :: exe_path = &
            '/tmp/ffc_assumed_rank_select_rank3_oracle'
        character(len=*), parameter :: out_path = &
            '/tmp/ffc_assumed_rank_select_rank3_oracle.out'
        ...
        test_gfortran_oracle = index(actual, '1 12 23 42') == 1
    ...
    logical function test_refusals()
        ...
        character(len=*), parameter :: allocatable_actual = &
            ...
            '  end subroutine work'//new_line('a')// &
            'end program main'
        character(len=*), parameter :: section_actual = &
            'program main'//new_line('a')// &
        ...
        character(len=*), parameter :: missing_arm = &
            ...
            '  end subroutine work'//new_line('a')// &
            'end program main'
        character(len=*), parameter :: rank_four = &
            'program main'//new_line('a')// &
        ...
        if (.not. expect_error_contains(scalar_actual, 'only a whole array is supported', &
                '/tmp/ffc_assumed_rank_rank3_scalar_reject')) test_refusals = .false.
        if (.not. expect_error_contains(dynamic_shape, 'dynamic shapes are not supported', &
                '/tmp/ffc_assumed_rank_rank3_dynamic_reject')) test_refusals = .false.
        if (.not. expect_error_contains(pointer_actual, 'pointer and alias actuals are refused', &
                '/tmp/ffc_assumed_rank_rank3_pointer_reject')) test_refusals = .false.
        if (.not. expect_error_contains(allocatable_actual, 'only a whole array is supported', &
                '/tmp/ffc_assumed_rank_rank3_allocatable_reject')) test_refusals = .false.
        if (.not. expect_error_contains(section_actual, 'only a whole array is supported', &
                '/tmp/ffc_assumed_rank_rank3_section_reject')) test_refusals = .false.
        ...
        if (.not. expect_error_contains(rank_four, 'only a rank-1, rank-2, or rank-3 whole REAL array', &
                '/tmp/ffc_assumed_rank_rank3_rank4_reject')) test_refusals = .false.
...
end program test_session_assumed_rank_select_rank3_compiler
