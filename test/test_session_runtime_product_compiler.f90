program test_session_runtime_product_compiler
    ...
    character(len=*), parameter :: source = &
        ...
        '  rank4_input = 2'//new_line('a')// &
        '  call consume4(rank4_input)'//new_line('a')// &
        '  call automatic(2,2,2)'//new_line('a')// &
        '  call automatic_real(2,2,2)'//new_line('a')// &
        '  call automatic4(2,2,2,2)'//new_line('a')// &
        '  call automatic4_real(2,2,2,2)'//new_line('a')// &
        '  call automatic64(3)'//new_line('a')// &
        '  deallocate(input)'//new_line('a')// &
        '  deallocate(real_input)'//new_line('a')// &
        '  deallocate(rank4_input)'//new_line('a')// &
        'contains'//new_line('a')// &
        ...
        '  subroutine consume4(values)'//new_line('a')// &
        '    integer, intent(in) :: values(:,:,:,:)'//new_line('a')// &
        '    print *, product(values)'//new_line('a')// &
        '  end subroutine consume4'//new_line('a')// &
        ...
        '  subroutine automatic4(n,m,k,l)'//new_line('a')// &
        '    integer, intent(in) :: n,m,k,l'//new_line('a')// &
        '    integer :: values(n,m,k,l)'//new_line('a')// &
        '    values = 2'//new_line('a')// &
        '    print *, product(values)'//new_line('a')// &
        '  end subroutine automatic4'//new_line('a')// &
        '  subroutine automatic4_real(n,m,k,l)'//new_line('a')// &
        '    integer, intent(in) :: n,m,k,l'//new_line('a')// &
        '    real :: values(n,m,k,l)'//new_line('a')// &
        '    values = 2.0'//new_line('a')// &
        '    print *, product(values)'//new_line('a')// &
        '  end subroutine automatic4_real'//new_line('a')// &
...
contains
    ...
    logical function test_rank5_refusal(program_source) result(ok)
        ...
        character(len=*), parameter :: base = &
            '/var/tmp/ert/ffc_runtime_product_rank5_refusal'
        ...
        call execute_command_line('gfortran -w '//src//' -o '//ref, &
            exitstat=exit_stat)
        if (exit_stat /= 0) then
            print *, 'FAIL: gfortran rejected valid rank-5 product refusal fixture'
        ...
        call execute_command_line('rm -f '//src//' '//ref//' '//exe)
        if (len_trim(error_msg) == 0) then
            print *, 'FAIL: rank-5 runtime product lowered without a diagnostic'
            return
        end if
        if (index(error_msg, &
            'runtime-sized array supports ranks 1 through 4 only') == 0) then
            print *, 'FAIL: rank-5 product refusal diagnostic changed: ', &
                trim(error_msg)
            return
        end if
        ok = .true.
