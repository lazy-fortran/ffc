...
contains
    ...
    logical function emit_f32_array_alloca(session, array_size, address, &
        ...
        vreg = lr_session_emit(session%handle, inst, error)
        if (.not. status_ok(error%code, error, error_msg)) then
            error_msg = 'f32_array_alloca: '//error_msg
            return
        end if

        address = ptr_vreg(session, vreg)
        call set_empty(error_msg)
        emit_f32_array_alloca = .true.
    ...
    logical function emit_f32_array_element_addr(session, array_size, &
        ...
        emit_f32_array_element_addr = .false.
        if (.not. require_open_session(session, error_msg)) return
        ...
        vreg = lr_session_emit(session%handle, inst, error)
        if (.not. status_ok(error%code, error, error_msg)) then
            error_msg = 'f32_gep: '//error_msg
            return
        end if

        element_addr = ptr_vreg(session, vreg)
        call set_empty(error_msg)
        emit_f32_array_element_addr = .true.
    ...
    logical function emit_f64_array_alloca(session, array_size, address, &
        ...
        vreg = lr_session_emit(session%handle, inst, error)
        if (.not. status_ok(error%code, error, error_msg)) then
            error_msg = 'f64_array_alloca: '//error_msg
            return
        end if

        address = ptr_vreg(session, vreg)
        call set_empty(error_msg)
        emit_f64_array_alloca = .true.
    ...
    logical function emit_f64_array_element_addr(session, array_size, &
        ...
        emit_f64_array_element_addr = .false.
        if (.not. require_open_session(session, error_msg)) return
