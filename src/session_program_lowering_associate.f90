...
contains
    ...
    subroutine bind_associate_name(arena, assoc, association_index, scope_index, &
        ...
        if (associate_selector_is_array(arena, assoc%expr_index, context)) then
            ...
            if (expression_rank == 1) then
                ...
                expression_dim2 = context%symbols(expression_source)%array_dim_sizes(2)
                expression_dim3 = context%symbols(expression_source)%array_dim_sizes(3)
                expression_dim4 = context%symbols(expression_source)%array_dim_sizes(4)
                expression_extent = expression_dim1 * expression_dim2 * expression_dim3 * &
                    expression_dim4
            end if
            ...
            if (expression_rank == 2) then
                ...
                context%symbols(expression_symbol)%array_dim_lowers(3) = 1
                context%symbols(expression_symbol)%array_dim_sizes(3) = expression_dim3
                context%symbols(expression_symbol)%array_dim_lowers(4) = 1
                context%symbols(expression_symbol)%array_dim_sizes(4) = expression_dim4
            end if
    ...
    subroutine materialize_scalar_associate_source(context, symbol_index, &
        ...
        character(len=:), allocatable, intent(out) :: error_msg
        type(lr_operand_desc_t) :: current_value
        logical :: ok

        call set_empty(error_msg)
        if (context%symbols(symbol_index)%has_address) return
        if (context%symbols(symbol_index)%is_array .or. &
            context%symbols(symbol_index)%is_derived) return

        select case (context%symbols(symbol_index)%value_kind)
        case (VALUE_I32, VALUE_LOGICAL)
            ok = emit_i32_alloca(context%session, &
                context%symbols(symbol_index)%address, error_msg)
        case (VALUE_I64)
            ok = emit_i64_alloca(context%session, &
                context%symbols(symbol_index)%address, error_msg)
        case (VALUE_I8)
            ok = emit_i8_alloca(context%session, &
                context%symbols(symbol_index)%address, error_msg)
        case (VALUE_I16)
            ok = emit_i16_alloca(context%session, &
                context%symbols(symbol_index)%address, error_msg)
        case (VALUE_F32)
            ok = emit_liric_f32_alloca(context%session, &
                context%symbols(symbol_index)%address, error_msg)
        case (VALUE_F64)
            ok = emit_liric_f64_alloca(context%session, &
                context%symbols(symbol_index)%address, error_msg)
        case default
            return
        end select
        ...
        current_value = context%symbols(symbol_index)%value
        call store_reference_value(context, symbol_index, &
    ...
    subroutine bind_associate_component(arena, comp_node, assoc_name, binding, &
        ...
        if (component_access_is_alloc_array(arena, comp_node, context)) then
            call bind_associate_alloc_array_component(arena, comp_node, &
                assoc_name, binding, context, error_msg)
            return
        end if
    ...
    subroutine bind_associate_alloc_array_component(arena, comp_node, assoc_name, &
        ...
        ! uses the same flat i32-slot layout as an ordinary array symbol.
        type(ast_arena_t), intent(in) :: arena
        type(component_access_node), intent(in) :: comp_node
        character(len=*), intent(in) :: assoc_name
        type(declaration_binding_t), intent(in) :: binding
        ...
        integer :: base_type, component_index, component_type
        integer :: comp_kind, idx
        ...
        if (component_alloc_rank_at(context, base_type, component_index) == 2) then
            call unsupported_feature_error('associate component selector', &
                comp_node%line, comp_node%column, &
        ...
        if (comp_kind == VALUE_CHARACTER) then
            call unsupported_feature_error('associate component selector', &
                comp_node%line, comp_node%column, &
                'ffc direct-session associate does not support a character '// &
                'array component selector', error_msg)
            return
        end if
        call lower_derived_component_address(arena, comp_node, context, &
        ...
        context%symbols(idx)%is_pointer = .false.
        context%symbols(idx)%is_parameter = .false.
        context%symbols(idx)%is_derived = component_type > 0
        context%symbols(idx)%derived_type_index = component_type
        context%symbols(idx)%is_deferred_character = .false.
        context%symbols(idx)%is_array = .true.
        context%symbols(idx)%array_rank = 1
        ! The descriptor supplies the dynamic extent. A positive nominal GEP
        ! extent is still required by the direct LIRIC array-address helpers;
        ! the generated index remains the actual runtime slot offset.
        context%symbols(idx)%array_size = 1
        context%symbols(idx)%array_lower_bound = 1
        context%symbols(idx)%array_dim_lowers(1) = 1
        context%symbols(idx)%array_dim_sizes(1) = 1
        context%symbols(idx)%is_allocatable = .false.
        call attach_symbol_binding(context, idx, binding)
        call set_empty(error_msg)
