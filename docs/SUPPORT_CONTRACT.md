...
| Derived types | Simple program-scope derived type definitions with scalar integer components and fixed-size integer 1-D array components (`integer :: a(N)`, N a literal or integer parameter). Scalar variables of those types are supported. Component assignment, component reads, array-component element access (`x%a(i)`), `print`, and `stop` are supported. Constructors, inheritance, type parameters, type-bound procedures, nested derived types, arrays of derived-type instances, whole-derived and whole-array-component assignment, and non-integer components are unsupported. |
