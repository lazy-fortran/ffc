...
| Interfaces | An `interface` block declaring `bind(c)` integer functions is parsed and registered (Fortran name, C name, argument and result kinds). Calls to such functions lower to a direct call on the C symbol with by-value integer arguments. Non-`bind(c)`, generic, operator, and non-integer interface procedures are rejected. |
