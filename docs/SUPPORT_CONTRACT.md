...
| `select case` | Single-arm or multi-arm `SELECT CASE` with literal integer labels, multi-label arms (`case (1, 5)`), and `case default`. Each arm body must terminate with `stop` or `return`. Non-terminating fallthrough is tracked under #180. |
...
| Intrinsics | Scalar `abs`, `min`, `max`, and `mod` for integer values; `abs`, `min`, `max` for real values; integer-to-real `real()` conversion. These lower inline with LIRIC scalar operations and control-flow PHI values. |
...
| Arrays | Fixed-size one-dimensional integer arrays with compile-time integer bounds. Scalar integer parameters and explicit `lower:upper` bounds are supported. Element assignment, element reads, `print`, `stop`, and counted-loop subscripts are supported. Runtime bounds checks are not emitted. |
