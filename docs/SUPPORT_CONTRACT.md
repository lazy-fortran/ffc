...
| Modules | A module defining named constants and derived-type definitions compiles (the constants fold at use sites; the types are layout-only). `ffc -c` on such a source writes a sibling `<modulename>.fmod` describing the exports (see `docs/RUNTIME_ABI.md` module artefact format). Module variables, module procedures, and reading a `.fmod` on `use` are not yet supported. |
