...
| Interfaces | An `interface` block declaring `bind(c)` integer functions or `void` subroutines is parsed and registered (Fortran name, C name, argument and result kinds). Calls lower to a direct call on the C symbol with by-value arguments; an integer actual is passed as `i32`, and a `c_null_ptr` actual is passed as a null pointer (8 bytes). Non-`bind(c)`, generic, operator, and non-integer-result function interfaces are rejected. |
