...
| Interfaces | An `interface` block declaring `bind(c)` integer functions is parsed and the procedures registered (Fortran name, C name, argument and result kinds). Calls to them are not yet lowered. Non-`bind(c)`, generic, operator, and non-integer interface procedures are rejected. |
